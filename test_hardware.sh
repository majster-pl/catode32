...
try:
    ...
    print(config.BUTTON_HOLD_TIME_MS)
...
HOLD_TIME_MS=$(echo "$CONFIG" | sed -n '11p')
...
echo "Long press threshold: ${HOLD_TIME_MS}ms"
echo ""
...
print('Long press threshold: {}ms'.format(HOLD_TIME_MS))
    ...
    while True:
        ...
        for name, pin in buttons.items():
            ...
            # Button being held
                ...
                if not long_press_triggered[name]:
                    ...
                    if hold_duration >= HOLD_TIME_MS:
                        print(name, 'LONG PRESSED (held for {}ms)'.format(hold_duration))
            ...
            # Button released
                ...
                hold_duration = time.ticks_diff(current_time, press_start_time[name])
                if not long_press_triggered[name]:
                    print(name, 'released (held for {}ms)'.format(hold_duration))
                else:
                    print(name, 'released after long press')
                long_press_triggered[name] = False
